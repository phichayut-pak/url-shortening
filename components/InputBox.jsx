...
const InputBox = ({ className, onSubmitLink }) => {
  ...
  return (
    <div id="input_bg" className={`${classes.input_bg} absolute left-0 right-0 ml-auto mr-auto w-11/12 lg:w-3/4 -bottom-[4.5rem] lg:-bottom-[5.5rem] py-5 lg:py-16 bg-dark-violet px-7 lg:px-16 rounded-md`}>
      <form onSubmit={submitHandler} className='relative flex flex-col lg:flex-row justify-start items-center space-x-0 space-y-3 lg:space-x-3 lg:space-y-0'>
        ...
        <label htmlFor="input_link" className={`lg:absolute lg:-bottom-5 lg:-left-3 font-poppins font-medium text-xs text-red w-full italic ${!isValid.status ? '' : 'hidden'}`}>{isValid.message}</label>
